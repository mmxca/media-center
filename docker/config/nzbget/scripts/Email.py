...
required_options = ('NZBPO_FROM', 'NZBPO_TO', 'NZBPO_SERVER', 'NZBPO_PORT',
                    'NZBPO_ENCRYPTION', 'NZBPO_USERNAME', 'NZBPO_PASSWORD')
...
total_status = os.environ.get(
    'NZBPP_TOTALSTATUS') if not test_mode else 'SUCCESS'
...
if (os.environ.get('NZBPO_STATISTICS') == 'yes' or
    ...
    rpcUrl = 'http://%s:%s@%s:%s/xmlrpc' % (
        quote(username), quote(password), host, port)
...
if os.environ.get('NZBPO_STATISTICS') == 'yes' and not test_mode:
    ...
    text += '\nDownload time: ' + \
        format_time_sec(int(nzbGroup['DownloadTimeSec']))
    text += '\nVerification time: ' + \
        format_time_sec(int(nzbGroup['ParTimeSec']) -
                        int(nzbGroup['RepairTimeSec']))
...
msg['Date'] = datetime.datetime.utcnow().strftime(
    "%a, %d %b %Y %H:%M:%S +0000")
...
try:
    if os.environ['NZBPO_ENCRYPTION'] == 'force':
        smtp = smtplib.SMTP_SSL(
            os.environ['NZBPO_SERVER'], os.environ['NZBPO_PORT'])
    else:
        smtp = smtplib.SMTP(
            os.environ['NZBPO_SERVER'], os.environ['NZBPO_PORT'])
    ...
    smtp.sendmail(os.environ['NZBPO_FROM'],
                  os.environ['NZBPO_TO'].split(','), msg.as_string())
